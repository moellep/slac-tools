...
def test_fully_populated_optional_array_stays_float64(tmp_path):
    o = _make_sample(rms_sizes=numpy.array([1.0, 2.0, 3.0]))
    r = _round_trip(o, tmp_path)
    _assert_equal(o, r)
...
def test_list_of_models_preserves_order_past_ten_elements(tmp_path):
    ...
    s = _make_sample(
        scans=[
            Detector(label=f"scan{i}", values=numpy.array([float(i)])) for i in range(12)
        ]
    )
    _assert_equal(s, _round_trip(s, tmp_path))
...
def test_nan_and_back(tmp_path):
    o = _make_sample(rms_sizes=numpy.array([1.25, numpy.nan]))
    _assert_equal(o, _round_trip(o, tmp_path))


def test_object_dtype_array_with_none_and_back(tmp_path):
    # dtype=object preserves None values
    o = _make_sample(rms_sizes=numpy.array([1.25, None], dtype=object))
    _assert_equal(o, _round_trip(o, tmp_path))


def test_optional_fields_omitted_when_none(tmp_path):
    o = _make_sample(optional_reading=None, notes=None, rms_sizes=None)
    _assert_equal(o, _round_trip(o, tmp_path))
...
def _make_sample(**overrides) -> Sample:
    defaults = dict(
        ...
        scans=[
            Detector(label=f"scan{i}", values=numpy.array([float(i)])) for i in range(11)
        ],
